...
export function createAutoTranslate(BdApi) {
const { React } = BdApi;
...
return class AutoTranslate {
  ...
  start() {
    ...
    for (const [name, method] of Object.entries({
      LocaleStore: "addChangeListener", UserStore: "getCurrentUser", ChannelStore: "getChannel",
      GuildStore: "getGuilds", SortedGuildStore: "getFlattenedGuildIds",
      MessageStore: "getMessage", SelectedChannelStore: "getChannelId",
    })) {
  ...
  stop() {
    ...
    this.reset();
    for (const id of this.subs.keys()) this.notify(id);
    this.subs.clear();
  ...
  async runBatch(batch) {
    ...
    if (!this.active || generation !== this.generation || target !== this.targetLang) return;
